...
// Shared state via window — survives module re-imports (pause → re-enable)
if (!window.__rb) {
  window.__rb = {
    apiRef: null,
    style: null,
...
export async function setup(api) {
  ...
  // Save original reloadPlugin on api object — survives pause/re-enable
  if (!api._rb_origReload) {
    api._rb_origReload = api.reloadPlugin;
  }
  ...
  api.reloadPlugin = async function(id) {
    ...
    return api._rb_origReload.call(api, id);
  };
  ...
  // Single global click handler — always reads from rb.apiRef (shared via window)
  if (!rb.clickHandlerBound) {
    document.addEventListener('click', (e) => {
      ...
      if (rbBtn && rb.apiRef) {
        ...
        openRollbackConfirm(rb.apiRef, rbBtn.dataset.rbRollback);
        return;
      }
    });
  ...
  console.log('\uD83D\uDD19 Rollback Manager v2.5.0 loaded');
}
...
async function performRollback(api, pluginId) {
  ...
  try {
    ...
    // Use the ORIGINAL reloadPlugin saved on api object
    await api._rb_origReload.call(api, pluginId);
    ...
    // Fetch remote version ourselves and set entry.remoteVersion
    // so PM's version comparison works (PM can't fetch from data: URLs)
    if (remoteUrl && !remoteUrl.startsWith('blob:') && !remoteUrl.startsWith('data:')) {
      try {
        const sep = remoteUrl.includes('?') ? '&' : '?';
        const res = await fetch(remoteUrl + sep + 't=' + Date.now());
        if (res.ok) {
          const code = await res.text();
          const metaMatch = code.match(/export const meta\s*=\s*(\{[\s\S]*?\})(?:;|$)/);
          if (metaMatch) {
            const remoteMeta = new Function('return ' + metaMatch[1])();
            if (remoteMeta.version) {
              const reg3 = api.registry.getAll();
              const ent3 = reg3.find(p => p.id === pluginId);
              if (ent3) {
                ent3.remoteVersion = remoteMeta.version;
                api.registry.save([...reg3]);
              }
            }
          }
        }
    ...
    // Force PM to re-render so update button appears
    setTimeout(() => {
      const pmRoot = document.querySelector('.pm-root');
...
export function teardown() {
  ...
  if (rb.apiRef) {
    rb.apiRef.reloadPlugin = rb.apiRef._rb_origReload || rb.apiRef.reloadPlugin;
  }
