...
function openLogViewer(api) {
  ...
  const eventColors = {
    'pm:install-start':'#007aff','pm:install-success':'#34c759','pm:install-fail':'#ff3b30',
    'pm:update-start':'#ff9500','pm:update-success':'#34c759','pm:update-fail':'#ff3b30',
    'pm:reload-start':'#5856d6','pm:reload-success':'#34c759','pm:reload-fail':'#ff3b30',
    'pm:retry-start':'#ff9500','pm:retry-success':'#34c759','pm:retry-fail':'#ff3b30',
    'pm:toggle':'#8e8e93','pm:delete':'#ff3b30','pm:status-change':'#5856d6',
    'pm:loaded':'#007aff','logger:loaded':'#5856d6'
  };
  ...
  overlay.innerHTML = `
    ...
    ">
      <div style="padding:20px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--pm-border)">
        ...
        <div id="pm-log-actions" style="display:flex;gap:10px">
          <button id="pm-log-clear" style="
            ...
            color:#ff3b30;padding:8px 14px;border-radius:10px;cursor:pointer">
            Clear
          </button>
          <button id="pm-log-close" style="
            background:var(--pm-text);color:var(--pm-bg);
      ...
      <div style="flex:1;overflow:auto;background:var(--pm-bg-soft)">
        ${logs.length === 0
          ? `<div style="padding:60px;text-align:center;color:var(--pm-subtext)">No activity</div>`
